...
class SpaceNode: SKSpriteNode, SKPhysicsContactDelegate {
    ...
    func move(_ timer:TimeInterval){
        ...
        if Steps.count>0 {
            ...
            let newPosition = CGPoint(x:currentPosition.x + velocity.x * CGFloat(timer), y:currentPosition.y + velocity.y * CGFloat(timer))
            position = newPosition
        }
    }
